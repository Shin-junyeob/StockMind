...
StockMind/
├─ code/
│  ├─ main.sh                         # crawling/main.py & analysis/main.py 실행용 쉘 스크립트(엔트리포인트)
...
│     ├─ io_utils.py                  # 입출력 유틸 (CSV/Parquet 로드·저장 등)
│     ├─ keywords.py                  # 기사 본문에서 핵심 키워드 추출 (예: KeyBERT 기반)
│     ├─ nlp_models.py                # 요약·감정분석·키워드 모델 로딩 및 관리
│     ├─ processor.py                 # 전체 분석 파이프라인 제어 (본문→summary/sentiment/keywords)
│     ├─ sentiment.py                 # 감성 분류 (positive/neutral/negative)
│     ├─ settings.py                  # 분석 단계 설정 (모델 경로, 파라미터)
│     ├─ summarizer.py                # 본문 요약 생성
│     └─ main.py                      # analysis 단계 실행 진입점 (batch 처리, 파일별 실행)
├─ data/
